...
<head>
    ...
    <style>
        :root { --primary: #3b198f; --primary-hover: #2e1069; --border-color: #E5E7EB; --text-main: #1D1A22; --text-muted: #6B7280; --accent-red: #E11D48; }
...
<body>
    ...
    <div class="wrapper">
        ...
        <div class="info-row">
            ...
            <span class="info-value"><?= ((isset($sessionData['metode']) && $sessionData['metode'] == 'diantar') || ($metode ?? '') == 'diantar') ? 'Diantar via Maxim' : 'Ambil Sendiri' ?></span>
        </div>
        <div class="info-row">
            <span class="info-label">Catatan</span>
            <span class="info-value" style="font-weight: 400;"><?= !empty($sessionData['catatan']) ? esc($sessionData['catatan']) : (!empty($catatan) ? esc($catatan) : 'Tidak ada catatan.') ?></span>
        </div>
        ...
        <div class="menu-list">
            ...
            <?php if(!empty($cartItems)): ?>
                ...
                <div class="menu-item">
                    <img src="<?= base_url('assets/img/' . ($item['gambar'] ?? 'menu_1.png')) ?>" class="menu-img" alt="<?= esc($item['nama']) ?>" onerror="this.src='<?= base_url('img/menu_default.png') ?>'">
                    <div class="menu-details">
                        <div class="menu-name"><?= esc($item['nama']) ?></div>
                        <div class="menu-variant"><?= esc($item['nama_varian'] ?? 'Porsi') ?></div>
                    </div>
                    <div class="menu-price-qty">
                        <span class="menu-price">Rp<?= number_format($item['harga'] * $item['jumlah'], 0, ',', '.') ?></span>
                        <span class="menu-qty">x<?= esc($item['jumlah']) ?></span>
                    </div>
                </div>
                <?php endforeach; ?>
            <?php elseif(!empty($rows)): ?>
                ...
                <div class="menu-item">
                    <img src="<?= base_url('assets/img/menu_1.png') ?>" class="menu-img" alt="<?= esc($r['produk']['nama']) ?>" onerror="this.src='<?= base_url('img/menu_default.png') ?>'">
                    <div class="menu-details">
                        <div class="menu-name"><?= esc($r['produk']['nama']) ?></div>
                        <div class="menu-variant"><?= esc(!empty($r['varian']['nama_varian']) ? $r['varian']['nama_varian'] : ($r['produk']['satuan'] ?? 'Porsi')) ?></div>
                    </div>
                    <div class="menu-price-qty">
                        <span class="menu-price">Rp<?= number_format($r['subtotal'], 0, ',', '.') ?></span>
                        <span class="menu-qty">x<?= esc($r['jumlah']) ?></span>
                    </div>
                </div>
                <?php endforeach; ?>
            <?php else: ?>
